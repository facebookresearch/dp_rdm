...
def main(opt, unknown):
    ...
    # running as `python main_rdm.py`
    # (in particular `main_rdm.DataModuleFromConfig`)
    ...
    if opt.resume:
        ...
        if os.path.isfile(opt.resume):
            logdir = opt.resume.rsplit("/checkpoints/", 1)[0]
            ckpt = opt.resume
        else:
    ...
    try:
        ...
        default_logger_cfgs = {
            "wandb": {
                ...
                "params": {
                    ...
                    "offline": opt.debug,
                    "id": nowname,
                    "entity": "retrieval-augmentation",
                    "project": "private-ldm-retrieval-augmentation",
                }
            },
            "tensorboard": {
        ...
        if "logger" in lightning_config:
            ...
            trainer_kwargs["logger"] = instantiate_from_config(default_logger_cfgs[logger_cfg.name])
        else:
            logger_cfg = OmegaConf.create()
            default_logger_cfg = default_logger_cfgs[logger_cfg.name if "name" in logger_cfg else "testtube"]
            trainer_kwargs["logger"] = instantiate_from_config(logger_cfg)
        print('logger' , lightning_config.logger)
        
        # if type(lightning_config.logger) is str:
        ...
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "train_private_rdm.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "image_logger": {
                "target": "train_private_rdm.ImageLogger",
                "params": {
                    "batch_frequency": 750,
                    "max_images": 4,
                    "clamp": True
            ...
            "learning_rate_logger": {
                "target": "train_private_rdm.LearningRateMonitor",
                "params": {
            ...
            # "cuda_callback": {
            #     "target": "train_private_rdm.CUDACallback"
            # },
        }
        if version.parse(pl.__version__) >= version.parse('1.4.0'):
        ...
        def melk(*args, **kwargs):
            ...
            if trainer.global_rank == 0:
                ...
                trainer.save_checkpoint(ckpt_path)
            if callable(prev_handler_usr2):
                prev_handler_usr2(*args, **kwargs)
        ...
        prev_handler_usr2 = signal.getsignal(signal.SIGUSR2)
        print(prev_handler_usr2)

        def aggregate_handler_usr2(*args, **kwargs):
            melk(*args, **kwargs)
            if callable(prev_handler_usr2):
                prev_handler_usr2(*args, **kwargs)

        signal.signal(signal.SIGUSR2, aggregate_handler_usr2)
...
if __name__ == "__main__":
    ...
    #       key: value
    # data:
    #   target: main_rdm.DataModuleFromConfig
    #   params:
    #      batch_size: int
    #      wrap: bool
    #      train:
