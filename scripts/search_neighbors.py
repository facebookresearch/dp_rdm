...
def search_nns(dataset_builder, qloader, device='cuda', mode='img', save=False, npatches_perside=None, base_savedir=None, nn_paths=None, corrupts=None, start_id=0, max_its=None):
    ...
    for i, batch in enumerate(tqdm(qloader, desc='Searching nns and saving embeddings', total=len(qloader) if max_its is None else max_its)):
        ...
        results = dataset_builder.search_k_nearest(queries=query, visualize=False, is_caption=caption_sim)
...
if __name__ == '__main__':
    ...
    # setting some paths
    if len(dataset_builder.data_pool["embedding"]) > 0:
        prefix = dataset_builder.data_pool["embedding"].shape[0]
    else:
        prefix = 0
    base_savedir = os.path.join(
        opt.nns_savedir,
        f'{prefix}p-{dataset_builder.retriever_name}_{dataset_builder.patch_size}@{dataset_builder.dset_name}',
