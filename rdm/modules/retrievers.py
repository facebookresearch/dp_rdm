...
class ClipImageRetriever(nn.Module):
    def __init__(
        ...
        self.device=device
        print(clip_type, model, device)
        if clip_type == 'clip':
            print("WARNING: Something MIGHT be wrong, you should be using open_clip!!")
            self.model, _ = load_clip(name=model, device=device, jit=jit)
        if clip_type == 'open_clip':
            clip_model, _, _preprocess = open_clip.create_model_and_transforms(model, pretrained=pretrained, device=device)
    ...
    def forward(self, x):
        # x is assumed to be in range [-1,1]
        return self.model.encode_image(self.clip_preprocess(x))
